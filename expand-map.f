...
C
	program expandmap
C
C  This program expands the gravity coefficients into a map.
C    The output is in mgal, with quarter-degree spacing.
C
C  This program up for grid registration (not pixel).
C
	implicit none
	real*8 C1lm(96878),C2lm(96878),pi,th,dth,norm,GM
	real*8 N(96878),P(96878),lon,lat,r,j1,j2,j3,cth,sth,fi,g
	integer*4 l,lmin,lmax,m,i,j,k,dl
	character*30 inname,outname
C
	pi = dacos(-1.d0)
C
	...
	read(10,*)r,GM,j1,lmax,j,k,j2,j3
	g = (GM/(r*r))*1.d8
C	if (lmax.gt.100) lmax = 100
	print *," "
	print *,'Maximum degree-and-order will be ',lmax
	print *," "
C
C  Read in the coefficients from the file:
	do l = 2,lmax
	  do m = 0,l
	    i = l*(l+1)/2 + m
	    read(10,*)j,k,C1lm(i),C2lm(i),j1,j2
	    C1lm(i) = g*dble(l + 1)*C1lm(i)
	    C2lm(i) = g*dble(l + 1)*C2lm(i)
	  enddo
	enddo
	close(10)
C
...
C	read(*,*)lmin
	lmin = 10 
	print *,"Minimum fixed at 10 to remove regional trends."
	print *," "
C
...
C  The guts of this section are "barrowed" from Banerdt's code.
	do lat = -90.d0,90.d0,dth
	  th = pi*(90.d0 - lat)/180.d0
	  cth = dcos(th)
	  sth = dsin(th)
	  if (lmax.ge.2) then
...
********** calculate polynomials of 1<order<(l-1) for each degree
     ...
     *                     -dble(l+m-1)*P((l-2)*(l-1)/2+m))/dble(l-m)
	    enddo
	  enddo
	  do lon = 0.d0,360.d0 - dth,dth
	    fi = pi*lon/180.d0
	    r = N(1)*cth*C1lm(1) + 
     *          N(2)*sth*(C1lm(2)*dcos(fi) + C2lm(2)*dsin(fi))
	    if (lmax.ge.2) then
	      r = r + N(3)*P(3)*C1lm(3) + N(4)*P(4)*(C1lm(4)*dcos(fi) +
     *            C2lm(4)*dsin(fi)) + N(5)*P(5)*(C1lm(5)*dcos(2.d0*fi) +
     *            C2lm(5)*dsin(2.d0*fi))
	    endif
	    if (lmax.ge.3) then
	      r = r + N(6)*P(6)*C1lm(6) + N(7)*P(7)*(C1lm(7)* 
     *            dcos(fi) + C2lm(7)*dsin(fi)) + N(8)*P(8)*(C1lm(8)*
     *            dcos(2.d0*fi) + C2lm(8)*dsin(2.d0*fi)) + N(9)*P(9)*
     *            (C1lm(9)*dcos(3.d0*fi) + C2lm(9)*dsin(3.d0*fi))
	    endif
	    do l = 4,lmax
	      r = r + N(l*(l+1)/2+0)*P(l*(l+1)/2+0)*C1lm(l*(l+1)/2+0) + 
     ...
     *            dsin(dble(l)*fi))
	      do m = 2,l-2
	        r = r + N(l*(l+1)/2+m)*P(l*(l+1)/2+m)*(
     *              C1lm(l*(l+1)/2+m)*dcos(dble(m)*fi) + 
     *              C2lm(l*(l+1)/2+m)*dsin(dble(m)*fi))
	      enddo
	    enddo
	    write(11,666)lat,lon,r
	  enddo
	enddo
C
	close(unit=11)
666	format(2x,f7.3,2x,f7.3,2x,E18.12)
	end
C
C23456789012345678901234567890123456789012345678901234567890123456789012
C  The guts of this section are "barrowed" from Banerdt's code.
